   ...
   IF ! lSkipBuild .AND. ! hbmk[ _HBMK_lStopAfterInit ] .AND. ! hbmk[ _HBMK_lStopAfterHarbour ] .AND. ! hbmk[ _HBMK_lDumpInfo ]
      ...
      IF ! Empty( l_cBLDHEAD )
         ...
         hb_MemoWrit( l_cBLDHEAD, ;
            ...
            "#define _HBMK_BUILD_TIMESTAMP_UTC_ " + '"' + hb_TToS( hb_TSToUTC( tmp1 ) )       + '"' + hb_eol() )
      ENDIF
   ENDIF
...
FUNCTION hb_regexMatch( ... )  /* For a plugin that prefers to rely on buggy/deprecated API. See commit: 2009-03-10 17:25 UTC+0100 */
   RETURN iif( hb_defaultValue( hb_PValue( 5 ), .F. ), hb_regexLike( ... ), hb_regexHas( ... ) )
#endif
   ...
   IF ! Empty( cCommand )
      ...
      CASE _VCS_GIT
         ...
         IF Len( aResult := hb_ATokens( cStdOut, .T. ) ) >= 8
            ...
            hbmk[ _HBMK_tVCSTS ] := hb_SToT( hCustom[ "COMMIT_TIMESTAMP_UTC" ] ) + ( hb_UTCOffset() / 86400 )
