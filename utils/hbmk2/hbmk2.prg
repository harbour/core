...
STATIC PROCEDURE hbmk_local_entry( ... )
   ...
   IF ! Empty( aArgsProc )
      ...
      ELSEIF hb_RightEq( tmp, "10" )
         hb_AIns( aArgsProc, 1, "-hb10", .T. )
      ELSEIF hb_RightEq( tmp, "20" )
         hb_AIns( aArgsProc, 1, "-hb20", .T. )
      ELSEIF hb_RightEq( tmp, "30" )
...
STATIC PROCEDURE hbmk_harbour_dirlayout_init( hbmk )
   ...
   IF ! Empty( hbmk[ _HBMK_cHB_INSTALL_ADD ] := GetEnv( "HB_INSTALL_ADDONS" ) )
      ...
      FOR EACH tmp IN hb_ATokens( hbmk[ _HBMK_cHB_INSTALL_ADD ], hb_osPathListSeparator(), lDOSWinTokens, lDOSWinTokens )
         IF ! Empty( tmp )
            AAdd( hbmk[ _HBMK_aLIBPATH ], hb_PathNormalize( hb_DirSepAdd( hb_DirSepToOS( tmp ) ) ) )
         ENDIF
      NEXT
            ...
            IF ! hbmk[ _HBMK_lCLEAN ]
               ...
               IF ! lHBMAINDLLP
                  IF l_cMAIN != NIL
                     ...
                     AAdd( array, Upper( iif( hb_LeftEq( l_cMAIN, "@" ), SubStr( l_cMAIN, 2 ), l_cMAIN ) ) )
                  ENDIF
                  IF hbmk[ _HBMK_cGT ] != NIL
...
STATIC FUNCTION HBC_ProcessOne( hbmk, cFileName, nNestingLevel )
   ...
   FOR EACH cLine IN hb_ATokens( cFile, _CHR_EOL )
      ...
      CASE hb_LeftEq( cLineL, "safe="         ) ; cLine := SubStr( cLine, Len( "safe="         ) + 1 )
         DO CASE
      ...
      IF ! Empty( cLine )
         FOR EACH cWord IN rtlnk_tokens( cLine )
            ...
            ELSE
               cWord := Upper( cWord )
               IF Len( cWord ) >= 2
                  IF hb_LeftEq( "OUTPUT", cWord )
                     nMode := RTLNK_MODE_OUT
                  ELSEIF hb_LeftEq( "FILE", cWord )
                     nMode := RTLNK_MODE_FILE
                  ELSEIF hb_LeftEq( "LIBRARY", cWord )
                     nMode := RTLNK_MODE_LIB
                  ELSEIF hb_LeftEq( "MODULE", cWord ) .OR. ;
                         hb_LeftEq( "EXCLUDE", cWord ) .OR. ;
                         hb_LeftEq( "REFER", cWord ) .OR. ;
                         hb_LeftEq( "INTO", cWord )
                     nMode := RTLNK_MODE_SKIP
                  /* Blinker extension */
                  ELSEIF hb_LeftEq( "BLINKER", cWord )
                     /* skip Blinker commands */
                     EXIT
                  ELSEIF hb_LeftEq( "ECHO", cWord )
                     _hbmk_OutStd( hbmk, hb_StrFormat( I_( "Blinker ECHO: %1$s" ), SubStr( cLine, 6 ) ) )
                     EXIT
                  ELSEIF hb_LeftEq( "MAP", cWord )
                     hbmk[ _HBMK_lMAP ] := .T.
                     EXIT
                  ELSEIF hb_LeftEq( "NOBELL", cWord )
                     hbmk[ _HBMK_lBEEP ] := .F.
                     EXIT
...
STATIC PROCEDURE __hbshell( cFile, ... )
   ...
   LOCAL lDebug := ;
      ...
      hb_RightEqI( hb_FNameName( hb_argv( 0 ) ), "d" )
