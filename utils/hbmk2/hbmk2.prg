...
STATIC FUNCTION __hbmk( aArgs, nArgTarget, nLevel, /* @ */ lPause, /* @ */ lExitStr )
   ...
   IF HBMK_ISPLAT( "win|wce|dos|os2|linux" )
         ...
         AAdd( aCOMPDET_EMBED, { {| cPrefix | tmp1 := hb_PathNormalize( hb_DirSepAdd( hbmk[ _HBMK_cHB_INSTALL_PFX ] ) + _HBMK_SPECDIR_COMP + hb_ps() + "mingw"    + hb_ps() + "bin"     ), iif( hb_FileExists( tmp1 + hb_ps() + cPrefix + "gcc" + hbmk[ _HBMK_cCCEXT ] ), tmp1, NIL ) }, "win"  , "mingw32" , ""                    , NIL, {| cARCH, cCOMP, cPathBin | hbmk_COMP_Setup( cARCH, cCOMP, cPathBin + hb_ps() + ".." ) } } )
         AAdd( aCOMPDET_EMBED, { {| cPrefix | tmp1 := hb_PathNormalize( hb_DirSepAdd( hbmk[ _HBMK_cHB_INSTALL_PFX ] ) + _HBMK_SPECDIR_COMP + hb_ps() + "mingw"    + hb_ps() + "bin"     ), iif( hb_FileExists( tmp1 + hb_ps() + cPrefix + "gcc" + hbmk[ _HBMK_cCCEXT ] ), tmp1, NIL ) }, "win"  , "mingw"   , "i686-w64-mingw32-"   , NIL, NIL } )
         AAdd( aCOMPDET_EMBED, { {| cPrefix | tmp1 := hb_PathNormalize( hb_DirSepAdd( hbmk[ _HBMK_cHB_INSTALL_PFX ] ) + _HBMK_SPECDIR_COMP + hb_ps() + "mingw"    + hb_ps() + "bin"     ), iif( hb_FileExists( tmp1 + hb_ps() + cPrefix + "gcc" + hbmk[ _HBMK_cCCEXT ] ), tmp1, NIL ) }, "win"  , "mingw32" , "i686-w64-mingw32-"   , NIL, NIL } )
...
STATIC FUNCTION GetListOfHBDFiles( hbmk )
   ...
   FOR EACH aFile IN Directory( hbmk[ _HBMK_cHB_INSTALL_DOC ] + hb_ps() + "*.hbd" )
 ...
 *
 * Copyright 2012-2015 Viktor Szakats (vszakats.net/harbour)
 */
...
STATIC PROCEDURE ShowHeader( hbmk )
   ...
   IF hbmk[ _HBMK_lShellMode ]
      cText := ;
         ...
         "Copyright © 2007-2015, Viktor Szakáts" + e"\n" + ;
   ...
   ELSE
      ...
      cText += " " + HBRawVersion() + e"\n" + ;
         "Copyright © 1999-2015, Viktor Szakáts" + e"\n"
#ifdef HARBOUR_SUPPORT
   ENDIF
#endif
