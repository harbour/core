...
STATIC FUNCTION __hbmk( aArgs, nArgTarget, nLevel, /* @ */ lPause, /* @ */ lExitStr )
   ...
   FOR EACH aParam IN aParams
      ...
      CASE Left( cParamL, Len( "-seqhead=" ) ) == "-seqhead="

         l_cSEQHEAD := PathMakeAbsolute( hb_DirSepToOS( MacroProc( hbmk, SubStr( cParam, Len( "-seqhead=" ) + 1 ), aParam[ _PAR_cFileName ] ) ), aParam[ _PAR_cFileName ] )
         IF Empty( hb_FNameExt( l_cSEQHEAD ) )
            l_cSEQHEAD := hb_FNameExtSet( l_cSEQHEAD, ".ch" )
         ENDIF
   ...
   IF ! lSkipBuild .AND. ! hbmk[ _HBMK_lStopAfterInit ] .AND. ! hbmk[ _HBMK_lStopAfterHarbour ] .AND. ! hbmk[ _HBMK_lDumpInfo ]
      ...
      IF ! Empty( l_cSEQHEAD )
         IF hbmk[ _HBMK_lInfo ]
            _hbmk_OutStd( hbmk, hb_StrFormat( I_( "Creating sequence header: %1$s" ), l_cVCSHEAD ) )
         ENDIF
         /* We can use native EOLs because we never read this file back. */
         hb_MemoWrit( l_cSEQHEAD, ;
            "/* Automatically generated by " + _SELF_NAME_ + ". Do not edit. */" + hb_eol() + ;
            "#define _HBMK_SEQ_ID_ " + '"' + SeqID( l_cSEQHEAD, "_HBMK_SEQ_ID_" ) + '"' + hb_eol() )
      ENDIF
      IF ! Empty( l_cTSHEAD )
...
STATIC FUNCTION VCSID( hbmk, cDir, cVCSHEAD, /* @ */ cType )
   ...
   CASE _VCS_GIT_SUB
      ...
      IF nType == _VCS_GIT_SUB
         cOldDir := hb_cwd( cDir )
      ENDIF
      cCommand := "git" + iif( nType == _VCS_GIT_SUB .OR. Empty( cDir ), "", " --git-dir=" + cDir + ".git" ) + " rev-parse --short HEAD"
      EXIT
   CASE _VCS_MERCURIAL
      cType := "mercurial"
   ...
   IF ! Empty( cCommand )
      ...
      CASE _VCS_GIT
         /* fe3bb56<n> */
         cResult := hb_StrReplace( cStdOut, Chr( 13 ) + Chr( 10 ) )
         EXIT
      CASE _VCS_MERCURIAL
...
/*    { "-cpp=<value>"       , I_( "select C++ mode or standard. Allowed values are: def, yes, no, iso98, iso03, iso0x, gnu98, gnu0x" ) }, */;
      ...
      { "-seqhead=<file>"    , I_( "generate .ch header file with a sequential number, that will be rolled automatically on each build. Generated header will define preprocessor constant _HBMK_SEQ_ID_." ) }, ;
