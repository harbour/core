 ...
 */
static void hb_gt_wvw_DrawBoxRecessed( PWVW_WIN wvw_win, int iTop, int iLeft, int iBottom, int iRight, HB_BOOL fTight )
{
...
/* difference with gtwvt's:
   ...
   TODO: combine it with aOffset like DrawImage ?
 */
static void hb_gt_wvw_DrawBoxRaised( PWVW_WIN wvw_win, int iTop, int iLeft, int iBottom, int iRight, HB_BOOL fTight ) /* <-- none in gtwvt */
{
   PWVW_GLO wvw = hb_gt_wvw();
...
static void hb_gt_wvw_DrawOutline( PWVW_WIN wvw_win, int iTop, int iLeft, int iBottom, int iRight )
{
...
HB_FUNC( WVW_DRAWBOXGET )
{
   ...
   if( wvw && wvw_win )
   {
      ...
      xy      = hb_gt_wvw_GetXYFromColRow( wvw_win, usCol + usLen, usRow + 1 );
...
HB_FUNC( WVW_DRAWBOXRAISED )
{
   ...
   if( wvw_win )
   {
      ...
      hb_gt_wvw_DrawBoxRaised( wvw_win, iTop, iLeft, iBottom, iRight, fTight );
...
HB_FUNC( WVW_DRAWBOXRECESSED )
{
   ...
   if( wvw_win )
   {
      ...
      hb_gt_wvw_DrawBoxRecessed( wvw_win, iTop, iLeft, iBottom, iRight, fTight );
...
HB_FUNC( WVW_DRAWLABELEX )
{
   ...
   if( wvw && wvw_win && iSlot >= 0 && iSlot < ( int ) HB_SIZEOFARRAY( wvw->a.hUserFonts ) && wvw->a.hUserFonts[ iSlot ] )
   {
      ...
      int      oldTextAlign = SetTextAlign( wvw_win->hdc, hb_parnidef( 5, TA_LEFT ) );
      COLORREF oldTextColor = SetTextColor( wvw_win->hdc, ( COLORREF ) hb_parnintdef( 6, wvw_win->foreground ) );
      COLORREF oldBkColor   = SetBkColor( wvw_win->hdc, ( COLORREF ) hb_parnintdef( 7, wvw_win->background ) );
      HFONT    oldFont      = ( HFONT ) SelectObject( wvw_win->hdc, wvw->a.hUserFonts[ iSlot ] );
...
HB_FUNC( WVW_DRAWLABELOBJ )
{
   ...
   if( wvw_win )
   {
      ...
      COLORREF oldTextColor = SetTextColor( wvw_win->hdc, ( COLORREF ) hb_parnintdef( 9, wvw_win->foreground ) );
      COLORREF oldBkColor   = SetBkColor( wvw_win->hdc, ( COLORREF ) hb_parnintdef( 10, wvw_win->background ) );
      HFONT    oldFont      = ( HFONT ) SelectObject( wvw_win->hdc, ( HFONT ) HB_PARHANDLE( 11 ) );
...
HB_FUNC( WVW_DRAWGRIDHORZ )
{
   ...
   if( wvw && wvw_win )
   {
      ...
      for( i = 0; i < iRows; i++ )
      {
         int y = ( usAtRow * hb_gt_wvw_LineHeight( wvw_win ) ) + wvw_win->iTBHeight;
...
HB_FUNC( WVW_DRAWBUTTON )
{
   ...
   if( wvw && wvw_win && wvw_zer )
   {
      ...
      if( HB_ISCHAR( 6 ) )
      {
         ...
         oldTextAlign = SetTextAlign( wvw_win->hdc, TA_CENTER | TA_TOP );
...
HB_FUNC( WVW_DRAWTEXTBOX )
{
   ...
   if( wvw_win )
   {
      ...
      int      oldTextAlign = SetTextAlign( wvw_win->hdc, TA_TOP | TA_LEFT | TA_NOUPDATECP );
      COLORREF oldTextColor = SetTextColor( wvw_win->hdc, ( COLORREF ) hb_parnintdef( 10, wvw_win->foreground ) );
      COLORREF oldBkColor   = SetBkColor( wvw_win->hdc, ( COLORREF ) hb_parnintdef( 11, wvw_win->background ) );
      int      oldBkMode    = SetBkMode( wvw_win->hdc, hb_parnidef( 12, OPAQUE ) );
      HFONT    oldFont      = ( HFONT ) SelectObject( wvw_win->hdc, ( HFONT ) HB_PARHANDLE( 13 ) );
