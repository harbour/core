...
void CheckStatics( void );         /* creates the _STATICS function if there are any static defined */
...
void PopDefId( char * szVarName );      /* generates the pcode to pop a default value from the virtual machine stack onto a variable */
...
WORD wStatics = 0;         /* number of defined statics variables on the PRG */
...
Main       : { Line(); } Source       { CheckStatics(); if( ! _iQuiet ) printf( "\nsyntax ok\n" ); }
           ;
...
VarDefs    : LOCAL { iVarScope = VS_LOCAL; Line(); } VarList Crlf   { SetFrame(); }
           | STATIC { iVarScope = VS_STATIC; Line(); } VarList Crlf { wStatics += $3; }
           ;
...
VarDef     : IDENTIFIER                              { AddVar( $1 ); }
           | IDENTIFIER INASSIGN Expression          { AddVar( $1 ); PopDefId( $1 ); }
...
void FunDef( char * szFunName, char cScope )  /* stores a Clipper defined function */
{
   ...
   if( ! strcmp( szFunName, "_INITSTATICS" ) ) /* is it the _INITSTATICS function ? */
   {
      PushInteger( wStatics );
      wSymbol = GetSymbolPos( "_INITSTATICS" ) - ( _iStartProc ? 1: 2 );
      GenPCode3( _STATICS, LOBYTE( wSymbol ), HIBYTE( wSymbol ) );
   }
   else
   ...
   while( pFunc )
   {
      ...
      while( lPCodePos < pFunc->lPCodePos )
      {
         switch( pFunc->pCode[ lPCodePos ] )
         {
            ...
            case _POPDEFSTAT:
                 {
                   SHORT wVar = pFunc->pCode[ lPCodePos + 1 ] +
                                ( pFunc->pCode[ lPCodePos + 2 ] * 256 );
                   fprintf( yyc, "                _POPDEFSTAT, %i, %i,\t/* %s */\n",
                            pFunc->pCode[ lPCodePos + 1 ],
                            pFunc->pCode[ lPCodePos + 2 ],
                            GetVar( pFunc->pStatics, wVar )->szName );
                   lPCodePos += 3;
                 }
                 break;
            ...
            case _SFRAME: /* we only generate it if there are statics in the PRG */
                 if( wStatics )
                 {
                    w = GetSymbolPos( "_INITSTATICS" ) - ( _iStartProc ? 1: 2 );
...
void PopDefId( char * szVarName ) /* generates the pcode to pop a default value from the virtual machine stack onto a variable */
{
   ...
   if( functions.pLast && ( wVar = GetVarPos( functions.pLast->pLocals, szVarName ) ) )
      GenPCode3( _POPLOCAL, LOBYTE( wVar ), HIBYTE( wVar ) );

   else if( functions.pLast && ( wVar = GetVarPos( functions.pLast->pStatics, szVarName ) ) )
   {
      wVar += functions.pLast->wStaticsBase; /* number of already defined statics for this PRG */
      GenPCode3( _POPDEFSTAT, LOBYTE( wVar ), HIBYTE( wVar ) );
   }
...
void PopId( char * szVarName ) /* generates the pcode to pop a value from the virtual machine stack onto a variable */
{
   ...
   else if( ( wVar = GetVarPos( functions.pLast->pStatics, szVarName ) ) )
   {
      wVar += functions.pLast->wStaticsBase; /* number of already defined statics for this PRG */
...
{
   WORD wSym = GetSymbolPos( szSymbolName ); /* returns 1, 2, ... */
...
void CheckStatics( void ) /* creates the _STATICS function if there are any static defined */
{
   /* there are defined statics variables but no globals ones */
   if( wStatics && ! GetFunction( "_INITSTATICS" ) )
      FunDef( "_INITSTATICS", FS_INIT );
}
