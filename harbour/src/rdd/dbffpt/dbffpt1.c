...
static HB_ERRCODE hb_memoErrorRT( FPTAREAP pArea, HB_ERRCODE uiGenCode, HB_ERRCODE uiSubCode,
                                  char * szFileName, HB_ERRCODE uiOsCode, HB_USHORT uiFlags )
{
 ...
 */
static ULONG hb_fptGetMemoLen( FPTAREAP pArea, HB_USHORT uiIndex )
{
 ...
 */
static ULONG hb_fptStoreSMTItem( FPTAREAP pArea, PHB_ITEM pItem, HB_BYTE ** bBufPtr, HB_BOOL fTrans )
{
   ...
   switch( buffer[ 0 ] )
   {
      ...
      case SMT_IT_CHAR:
         ...
         *pfOffset += ulLen;
         if( fTrans && ulLen > 0 )
         {
            ULONG ulSize = ulLen + 1;
            hb_cdpnDup3( pBuffer, ulLen, pBuffer, &ulLen, &pBuffer, &ulSize,
            ...
            *pbMemoBuf += 2;
            ...
            else
            {
               ...
               *pbMemoBuf += ulLen;
               if( fTrans && ulLen > 0 )
               {
                  pszStr = hb_cdpnDup( pszStr, &ulLen, pArea->area.cdPage, hb_vmCDP() );
 ...
 */
static ULONG hb_fptStoreSixItem( FPTAREAP pArea, PHB_ITEM pItem, HB_BYTE ** bBufPtr, HB_BOOL fTrans )
{
   ULONG ulLen, u, ulSize;
   HB_MAXINT iVal;
   HB_LONG lVal;
   ...
   switch ( hb_itemType( pItem ) )
   {
      ...
      case HB_IT_MEMO:
         ...
         if( ulLen > 0 )
         {
            ...
            {
               hb_cdpnDup2( hb_itemGetCPtr( pItem ), ulLen,
                            ( char * ) *bBufPtr + SIX_ITEM_BUFSIZE, &u,
                            hb_vmCDP(), pArea->area.cdPage );
               ulLen = u;
            }
            else
                                 ...
                                 ULONG ulCount, HB_BOOL fTrans )
{
   ...
   if( ulBlock > 0 )
   {
      ...
      else if( pArea->bMemoType == DB_MEMO_DBT )
      {
         if( fTrans && ulSize != 0 )
         {
            ULONG ulBufSize = ulSize + 1;
            hb_cdpnDup3( pBuffer, ulSize, pBuffer, &ulSize,
                         &pBuffer, &ulBufSize,
                         pArea->area.cdPage, hb_vmCDP() );
         }
                                 ...
                                 ULONG * pulBlock, HB_BOOL fTrans )
{
   ...
   HB_BYTE  *bBufPtr = NULL, *bBufAlloc = NULL;
   HB_ERRCODE errCode;
   HB_MAXINT iVal;
   HB_LONG lVal;
   ...
   if( HB_IS_STRING( pItem ) )
   {
      ...
      if( ulSize > 0 && fTrans )
      {
         bBufPtr = bBufAlloc = ( HB_BYTE * )
                                 hb_cdpnDup( ( const char * ) bBufPtr, &ulSize,
                                             hb_vmCDP(), pArea->area.cdPage );
      }
         ...
         if( uiType <= HB_VF_CHAR ) /* 64000 max string size */
         {
            ...
            if( uiType <= pField->uiLen - 2 )
            {
               if( ulLen > 0 && ( pField->uiFlags & HB_FF_BINARY ) == 0 &&
                   hb_vmCDP() != pArea->area.cdPage )
               {
                  pString = pAlloc = hb_cdpnDup( ( const char * ) pFieldBuf, &ulLen,
                                                 pArea->area.cdPage, hb_vmCDP() );
               }
               else
            ...
            else
            {
               ...
               {
                  ulSize = ulLen + 1;
                  pString = hb_cdpnDup3( pString, ulLen, pAlloc, &ulLen,
                                         &pAlloc, &ulSize, pArea->area.cdPage, hb_vmCDP() );
               }
            }
...
static HB_ERRCODE hb_fptGetVarFile( FPTAREAP pArea, ULONG ulBlock, const char * szFile, HB_USHORT uiMode, HB_BOOL fTrans )
{
   HB_ERRCODE errCode;
   HB_FHANDLE hFile;
...
static HB_ERRCODE hb_fptPutVarField( FPTAREAP pArea, HB_USHORT uiIndex, PHB_ITEM pItem )
{
       ...
       pField->uiType == HB_FT_OLE )
   {
      ...
      {
         HB_BYTE buffer[ 11 ], *pBlock = NULL, *pAlloc = NULL;
         ...
         else if( HB_IS_STRING( pItem ) )
         {
            ...
            pBlock = ( HB_BYTE * ) hb_itemGetCPtr( pItem );
            if( ulLen > HB_VF_CHAR )
               ulLen = HB_VF_CHAR;
            if( ulLen > 0 && ( pField->uiFlags & HB_FF_BINARY ) == 0 &&
                hb_vmCDP() != pArea->area.cdPage )
 ...
 */
static HB_ERRCODE hb_fptGetVarLen( FPTAREAP pArea, HB_USHORT uiIndex, ULONG * pLength )
{
 ...
 */
static HB_ERRCODE hb_fptRddInfo( LPRDDNODE pRDD, HB_USHORT uiIndex, ULONG ulConnect, PHB_ITEM pItem )
{
   LPDBFDATA pData;
