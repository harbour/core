...
CLASS XbpBrowse INHERIT XbpWindow
   ...
   ACCESS   colCount                              INLINE    Len( ::columns )
   DATA     colPos                                INIT      1
   DATA     rowCount
   DATA     rowPos

   DATA     b_firstPosBlock                       PROTECTED
   DATA     b_goBottomBlock                       PROTECTED
   DATA     b_goPosBlock                          PROTECTED
   DATA     b_goTopBlock                          PROTECTED
   DATA     b_hitBottomBlock                      PROTECTED
   DATA     b_hitTopBlock                         PROTECTED
   DATA     b_lastPosBlock                        PROTECTED
   DATA     b_phyPosBlock                         PROTECTED
   DATA     b_posBlock                            PROTECTED
   DATA     b_skipBlock                           PROTECTED
   DATA     b_stableBlock                         PROTECTED

   METHOD   firstPosBlock                         SETGET
   METHOD   goBottomBlock                         SETGET
   METHOD   goPosBlock                            SETGET
   METHOD   goTopBlock                            SETGET
   METHOD   hitBottomBlock                        SETGET
   METHOD   hitTopBlock                           SETGET
   METHOD   lastPosBlock                          SETGET
   METHOD   phyPosBlock                           SETGET
   METHOD   posBlock                              SETGET
   METHOD   skipBlock                             SETGET
   METHOD   stableBlock                           SETGET

   METHOD   addColumn( oXbpColumn )
...
//   METHOD   forceStable()
   ...
   DATA     oVertHeaderView
   DATA     oHorzHeaderView
   DATA     pCurIndex
...
METHOD XbpBrowse:new( oParent, oOwner, aPos, aSize, aPresParams, lVisible )
...
METHOD XbpBrowse:create( oParent, oOwner, aPos, aSize, aPresParams, lVisible )
   ...
   ::oWidget := HbTableView():new( ::pParent )

   /* Do we need to fix the font size ? May be needed for TBrowse Behabvior */
   ::setFontCompoundName( "10.Courier" )
   ...
   /*  Hide veritical Header because of Performance boost */
   ::oVertHeaderView := QHeaderView()
   ::oVertHeaderView:configure( ::oWidget:verticalHeader() )
   ::oVertHeaderView:hide()

   ::oWidget:setModel( QT_PTROF( ::oDbfModel ) )
   ::oWidget:setCurrentIndex( QModelIndex():new():sibling( 0,0 ) )
   ::pCurIndex := ::oWidget:currentIndex()

   //::oWidget:setColumnWidth( 2, 150 )     // PP
   //::oWidget:setColumnWidth( 4, 30 )      // PP
   //::oWidget:setRowHeight( 0,70 )         // PP

   ::oWidget:setAlternatingRowColors( .t. )

   ::oWidget:setTabKeyNavigation( .t. )
   ::oWidget:setShowGrid( .t. )
   ::oWidget:setGridStyle( Qt_DotLine )   // to be based on column definition
   ::oWidget:setSelectionMode( QAbstractItemView_ExtendedSelection )

   ::oWidget:selectRow( 0 )
   // ::oWidget:resizeRowsToContents()  /* Very expansive */
...
METHOD XbpBrowse:supplyInfo( nInfo, p2, p3, p4 )
   LOCAL aColor
   ...
   CASE HBQT_BRW_COLHEIGHT
      RETURN IF( p3 > 0 .and. p3 <= ::colCount, ::columns[ p3 ]:height    , 12           )

   CASE HBQT_BRW_COLHEADER
      RETURN IF( p3 > 0 .and. p3 <= ::colCount, ::columns[ p3 ]:caption   , ""           )
   ...
   CASE HBQT_BRW_CELLVALUE
      /* Evaluate ROW Buffer */
      DbGoto( p2 )
      RETURN IF( p3 > 0 .and. p3 <= ::colCount, ::columns[ p3 ]:cellValue , ""           )
...
METHOD XbpBrowse:exeBlock( nEvent, p1 )
   LOCAL nXbpkey, pCurIndex

   DO CASE
   CASE nEvent == 1
      ::oModelIndex:configure( ::oWidget:currentIndex() )

      nXbpkey := XbpQKeyEventToAppEvent( p1 )

      DO CASE

      CASE nXbpKey == xbeK_RIGHT
         ::right()

      CASE nXbpKey == xbeK_LEFT
         ::left()

      CASE nXbpKey == xbeK_CTRL_PGUP
         ::oWidget:setCurrentIndex( ::oModelIndex:sibling( 0,::oModelIndex:column() ) )
         ::oWidget:scrollToTop()

      CASE nXbpKey == xbeK_CTRL_PGDN
         ::oWidget:setCurrentIndex( ::oModelIndex:sibling( lastRec()-1,::oModelIndex:column() ) )
         ::oWidget:scrollToBottom()

      OTHERWISE
         pCurIndex := ::oWidget:navigate( ConvertAFact( "BRWNAVIGATE", XBTOQT_FROM_XB, nXbpkey ) )
         ::oWidget:setCurrentIndex( pCurIndex )
         ::oWidget:scrollTo( ::oWidget:currentIndex() )
...
METHOD XbpBrowse:firstPosBlock( bBlock )
   IF hb_isBlock( bBlock )
      ::b_firstPosBlock := bBlock
   ENDIF
   RETURN ::b_firstPosBlock

/*----------------------------------------------------------------------*/

METHOD XbpBrowse:goBottomBlock( bBlock )
   IF hb_isBlock( bBlock )
      ::b_goBottomBlock := bBlock
   ENDIF
   RETURN ::b_goBottomBlock
...
METHOD XbpBrowse:hitBottomBlock( bBlock )
   IF hb_isBlock( bBlock )
      ::b_hitBottomBlock := bBlock
   ENDIF
   RETURN ::b_hitBottomBlock

/*----------------------------------------------------------------------*/

METHOD XbpBrowse:hitTopBlock( bBlock )
   IF hb_isBlock( bBlock )
      ::b_hitTopBlock := bBlock
   ENDIF
   RETURN ::b_hitTopBlock

/*----------------------------------------------------------------------*/

METHOD XbpBrowse:lastPosBlock( bBlock )
   IF hb_isBlock( bBlock )
      ::b_lastPosBlock := bBlock
   ENDIF
   RETURN ::b_lastPosBlock

/*----------------------------------------------------------------------*/

METHOD XbpBrowse:phyPosBlock( bBlock )
   IF hb_isBlock( bBlock )
      ::b_phyPosBlock := bBlock
   ENDIF
   RETURN ::b_phyPosBlock
...
METHOD XbpBrowse:skipBlock( bBlock )
   IF hb_isBlock( bBlock )
      ::b_skipBlock := bBlock
   ENDIF
   RETURN ::b_skipBlock
...
METHOD XbpBrowse:stableBlock( bBlock )
   IF hb_isBlock( bBlock )
      ::b_stableBlock := bBlock
   ENDIF
   RETURN ::b_stableBlock

/*----------------------------------------------------------------------*/

METHOD XbpBrowse:addColumn( oColumn )

   IF hb_isObject( oColumn )
      aadd( ::columns, oColumn )
xbp_Debug( "added column" )
...
METHOD XbpBrowse:left()
   ...
   IF ::colPos > 1
      ::colPos--
      ::pCurIndex := ::oWidget:navigate( QAbstractItemView_MoveLeft )
      ::oWidget:setCurrentIndex( ::pCurIndex )
      ::oWidget:scrollTo( ::oWidget:currentIndex() )
   ENDIF
...
METHOD XbpBrowse:right()
   ...
   IF ::colPos < ::colCount
      ::colPos++
      ::pCurIndex := ::oWidget:navigate( QAbstractItemView_MoveRight )
      ::oWidget:setCurrentIndex( ::pCurIndex )
      ::oWidget:scrollTo( ::oWidget:currentIndex() )
   ENDIF
...
METHOD XbpColumn:new( oParent, oOwner, aPos, aSize, aPresParams, lVisible )
   ...
   IF empty( ::picture )
      ...
      CASE ::valtype == "N"
         ...
         ELSE
            p := cVar
         ENDIF
         ::picture := "@Z "+replicate( "9", len( p ) ) + IF( n > 0, "."+ replicate( "9",len( l ) ), "" )
      CASE ::valtype == "D"
         ::picture := set( _SET_DATEFORMAT )
      CASE ::valtype == "L"
         ::picture := "Y"
      OTHERWISE
         ::picture := "@"
...
METHOD XbpColumn:cellValue()
   RETURN transform( ::setData(), ::picture )
