...
QT_G_FUNC( release_QNetworkRequest )
{
   ...
   if( p && p->ph )
   {
      ...
      HB_TRACE( HB_TR_DEBUG, ( "release_QNetworkRequest             Object deleted!" ) );
         ...
         hbqt_debug( "  YES release_QNetworkRequest             %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
      #endif
   }
   else
   {
      ...
      #if defined( __HB_DEBUG__ )
         hbqt_debug( "  DEL release_QNetworkRequest" );
      #endif
   }
}
...
void * gcAllocate_QNetworkRequest( void * pObj )
{
   ...
   #if defined( __HB_DEBUG__ )
      hbqt_debug( "          new_QNetworkRequest             %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
   #endif
   return( p );
}
