...
QT_G_FUNC( release_QPicture )
{
   ...
   if( p && p->ph )
   {
      ...
      HB_TRACE( HB_TR_DEBUG, ( "release_QPicture                    Object deleted!" ) );
         ...
         hbqt_debug( "  YES release_QPicture                    %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
      #endif
   }
   else
   {
      ...
      #if defined( __HB_DEBUG__ )
         hbqt_debug( "  DEL release_QPicture" );
      #endif
   }
}
...
void * gcAllocate_QPicture( void * pObj )
{
   ...
   #if defined( __HB_DEBUG__ )
      hbqt_debug( "          new_QPicture                    %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
   #endif
   return( p );
}
