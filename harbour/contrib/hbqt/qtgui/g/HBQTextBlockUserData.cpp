...
HBQT_GC_FUNC( hbqt_gcRelease_HBQTextBlockUserData )
{
   ...
   if( p && p->bNew )
   {
      if( p->ph )
      {
         HB_TRACE( HB_TR_DEBUG, ( "ph=%p    _rel_HBQTextBlockUserData   /.\\", p->ph ) );
         delete ( ( HBQTextBlockUserData * ) p->ph );
         HB_TRACE( HB_TR_DEBUG, ( "ph=%p YES_rel_HBQTextBlockUserData   \\./", p->ph ) );
         p->ph = NULL;
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "ph=%p DEL_rel_HBQTextBlockUserData    :     Object already deleted!", p->ph ) );
         p->ph = NULL;
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "ph=%p PTR_rel_HBQTextBlockUserData    :    Object not created with new=true", p->ph ) );
      p->ph = NULL;
   }
}
...
void * hbqt_gcAllocate_HBQTextBlockUserData( void * pObj, bool bNew )
{
   ...
   if( bNew )
   {
      HB_TRACE( HB_TR_DEBUG, ( "ph=%p    _new_HBQTextBlockUserData", pObj ) );
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "ph=%p NOT_new_HBQTextBlockUserData", pObj ) );
   }
   return p;
}
