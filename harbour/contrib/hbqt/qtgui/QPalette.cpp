...
QT_G_FUNC( release_QPalette )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "release_QPalette                     p=%p", p ) );
   HB_TRACE( HB_TR_DEBUG, ( "release_QPalette                    ph=%p", p->ph ) );

   if( p && p->ph )
   {
      delete ( ( QPalette * ) p->ph );
      p->ph = NULL;
      HB_TRACE( HB_TR_DEBUG, ( "YES release_QPalette                    Object deleted! %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   else
   {
