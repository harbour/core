...
QT_G_FUNC( release_QTextItem )
{
   ...
   if( p && p->ph )
   {
      ...
      HB_TRACE( HB_TR_DEBUG, ( "release_QTextItem                   Object deleted!" ) );
         ...
         hbqt_debug( "  YES release_QTextItem                   %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
      #endif
   }
   else
   {
      ...
      #if defined( __HB_DEBUG__ )
         hbqt_debug( "  DEL release_QTextItem" );
      #endif
   }
}
...
void * gcAllocate_QTextItem( void * pObj )
{
   ...
   #if defined( __HB_DEBUG__ )
      hbqt_debug( "          new_QTextItem                   %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
   #endif
   return( p );
}
