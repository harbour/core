...
QT_G_FUNC( release_QSizePolicy )
{
   ...
   if( p && p->ph )
   {
      ...
      HB_TRACE( HB_TR_DEBUG, ( "release_QSizePolicy                 Object deleted!" ) );
         ...
         hbqt_debug( "  YES release_QSizePolicy                 %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
      #endif
   }
   else
   {
      ...
      #if defined( __HB_DEBUG__ )
         hbqt_debug( "  DEL release_QSizePolicy" );
      #endif
   }
}
...
void * gcAllocate_QSizePolicy( void * pObj )
{
   ...
   #if defined( __HB_DEBUG__ )
      hbqt_debug( "          new_QSizePolicy                 %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
   #endif
   return( p );
}
