...
QT_G_FUNC( hbqt_gcRelease_QTimeEdit )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTimeEdit                    p=%p", p));
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTimeEdit                   ph=%p pq=%p", p->ph, (void *)(p->pq)));
   ...
   if( p && p->ph && p->pq )
   {
      ...
      if( ( QString ) m->className() != ( QString ) "QObject" )
      {
         ...
         case HBQT_RELEASE_WITH_DELETE_LATER:
            ( ( QTimeEdit * ) p->ph )->deleteLater();
            break;
         }
         p->ph = NULL;
         HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTimeEdit                   Object deleted! %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "NO hbqt_gcRelease_QTimeEdit                   Object Name Missing!" ) );
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "DEL hbqt_gcRelease_QTimeEdit                   Object Already deleted!" ) );
   }
}
...
void * hbqt_gcAllocate_QTimeEdit( void * pObj )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "          new_QTimeEdit                   %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   return p;
}
