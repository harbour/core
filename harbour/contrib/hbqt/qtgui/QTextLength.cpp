...
QT_G_FUNC( release_QTextLength )
{
   ...
   if( p && p->ph )
   {
      ...
      HB_TRACE( HB_TR_DEBUG, ( "release_QTextLength                 Object deleted!" ) );
         ...
         just_debug( "  YES release_QTextLength                 %i B %i KB", ( int ) hb_xquery( 1001 ), hb_getMemUsed() );
      #endif
   }
   else
   {
      ...
      #if defined(__debug__)
         just_debug( "  DEL release_QTextLength" );
      #endif
   }
}
...
void * gcAllocate_QTextLength( void * pObj )
{
   ...
   #if defined(__debug__)
      just_debug( "          new_QTextLength                 %i B %i KB", ( int ) hb_xquery( 1001 ), hb_getMemUsed() );
   #endif
   return( p );
}
