...
hb_snprintf( str, sizeof(str), "release_QStyleOptionSlider          %i B %i KB", ( int ) hb_xquery( 1001 ), hb_getMemUsed() );  OutputDebugString( str );
#endif
   void * ph = ( void * ) Cargo;
   if( ph )
   {
      ( ( QStyleOptionSlider * ) ph )->~QStyleOptionSlider();
      ph = NULL;
   }
   else
   {
#if defined(__debug__)
hb_snprintf( str, sizeof(str), "! ph____QStyleOptionSlider" );  OutputDebugString( str );
#endif
   }
}
...
HB_FUNC( QT_QSTYLEOPTIONSLIDER )
{
   ...
   void * pObj = NULL;
#if defined(__debug__)
hb_snprintf( str, sizeof(str), "   GC:  new QStyleOptionSlider          %i B %i KB", ( int ) hb_xquery( 1001 ), hb_getMemUsed() );  OutputDebugString( str );
#endif
