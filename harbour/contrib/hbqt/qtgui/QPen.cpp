...
QT_G_FUNC( hbqt_gcRelease_QPen )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QPen                         p=%p", p ) );
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QPen                        ph=%p", p->ph ) );

   if( p && p->ph )
   {
      delete ( ( QPen * ) p->ph );
      p->ph = NULL;
      HB_TRACE( HB_TR_DEBUG, ( "YES hbqt_gcRelease_QPen                        Object deleted! %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "DEL hbqt_gcRelease_QPen                        Object Already deleted!" ) );
   }
}
