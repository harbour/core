...
QT_G_FUNC( hbqt_gcRelease_QTextBlockFormat )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTextBlockFormat             p=%p", p ) );
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTextBlockFormat            ph=%p", p->ph ) );

   if( p && p->ph )
   {
      delete ( ( QTextBlockFormat * ) p->ph );
      p->ph = NULL;
      HB_TRACE( HB_TR_DEBUG, ( "YES hbqt_gcRelease_QTextBlockFormat            Object deleted! %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "DEL hbqt_gcRelease_QTextBlockFormat            Object Already deleted!" ) );
   }
}
