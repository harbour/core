...
typedef struct
{
  ...
  bool bNew;
  QT_G_FUNC_PTR func;
} QGC_POINTER_QStyleOptionTitleBar;
...
QT_G_FUNC( hbqt_gcRelease_QStyleOptionTitleBar )
{
   ...
   if( p && p->bNew )
   {
      if( p->ph )
      {
         ...
         HB_TRACE( HB_TR_DEBUG, ( "YES_rel_QStyleOptionTitleBar       ph=%p %i B %i KB", p->ph, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
         p->ph = NULL;
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "DEL_rel_QStyleOptionTitleBar        Object already deleted!" ) );
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "PTR_rel_QStyleOptionTitleBar        Object not created with - new" ) );
      p->ph = NULL;
   }
}
...
void * hbqt_gcAllocate_QStyleOptionTitleBar( void * pObj, bool bNew )
{
   ...
   if( bNew )
   {
      HB_TRACE( HB_TR_DEBUG, ( "   _new_QStyleOptionTitleBar       ph=%p %i B %i KB", pObj, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   return p;
}
...
HB_FUNC( QT_QSTYLEOPTIONTITLEBAR )
{
   ...
   hb_retptrGC( hbqt_gcAllocate_QStyleOptionTitleBar( pObj, true ) );
}
