...
QT_G_FUNC( hbqt_gcRelease_QStyleOptionProgressBar )
{
   ...
   if( p && p->bNew )
   {
      if( p->ph )
      {
         HB_TRACE( HB_TR_DEBUG, ( "ph=%p    _rel_QStyleOptionProgressBar   /.\\", p->ph ) );
         delete ( ( QStyleOptionProgressBar * ) p->ph );
         HB_TRACE( HB_TR_DEBUG, ( "ph=%p YES_rel_QStyleOptionProgressBar   \\./", p->ph ) );
         p->ph = NULL;
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "ph=%p DEL_rel_QStyleOptionProgressBar    :     Object already deleted!", p->ph ) );
         p->ph = NULL;
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "ph=%p PTR_rel_QStyleOptionProgressBar    :    Object not created with new=true", p->ph ) );
      p->ph = NULL;
   }
}
...
void * hbqt_gcAllocate_QStyleOptionProgressBar( void * pObj, bool bNew )
{
   ...
   if( bNew )
   {
      HB_TRACE( HB_TR_DEBUG, ( "ph=%p    _new_QStyleOptionProgressBar", pObj ) );
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "ph=%p NOT_new_QStyleOptionProgressBar", pObj ) );
   }
   return p;
}
