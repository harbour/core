...
QT_G_FUNC( hbqt_gcRelease_QContextMenuEvent )
{
   ...
   if( p && p->bNew )
   {
      if( p->ph )
      {
         ...
         HB_TRACE( HB_TR_DEBUG, ( "YES_rel_QContextMenuEvent          ph=%p %i B %i KB", p->ph, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
         p->ph = NULL;
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "DEL_rel_QContextMenuEvent           Object already deleted!" ) );
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "PTR_rel_QContextMenuEvent           Object not created with - new" ) );
      p->ph = NULL;
   }
}
...
void * hbqt_gcAllocate_QContextMenuEvent( void * pObj, bool bNew )
{
   ...
   if( bNew )
   {
      HB_TRACE( HB_TR_DEBUG, ( "   _new_QContextMenuEvent          ph=%p %i B %i KB", pObj, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   return p;
}
...
HB_FUNC( QT_QCONTEXTMENUEVENT )
{
   ...
   hb_retptrGC( hbqt_gcAllocate_QContextMenuEvent( pObj, true ) );
}
...
{
   hb_retptrGC( hbqt_gcAllocate_QPoint( new QPoint( hbqt_par_QContextMenuEvent( 1 )->globalPos() ), true ) );
}
...
{
   hb_retptrGC( hbqt_gcAllocate_QPoint( new QPoint( hbqt_par_QContextMenuEvent( 1 )->pos() ), true ) );
}
