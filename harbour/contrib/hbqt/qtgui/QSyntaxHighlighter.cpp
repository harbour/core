...
QT_G_FUNC( release_QSyntaxHighlighter )
{
   ...
   if( p && p->ph && p->pq )
   {
      ...
      if( ( QString ) m->className() != ( QString ) "QObject" )
      {
         ...
         p->ph = NULL;
         HB_TRACE( HB_TR_DEBUG, ( "release_QSyntaxHighlighter          Object deleted!" ) );
         #if defined( __HB_DEBUG__ )
            hbqt_debug( "  YES release_QSyntaxHighlighter          %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
         #endif
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "release_QSyntaxHighlighter          Object Name Missing!" ) );
         #if defined( __HB_DEBUG__ )
            hbqt_debug( "  NO  release_QSyntaxHighlighter" );
         #endif
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "release_QSyntaxHighlighter          Object Allready deleted!" ) );
      #if defined( __HB_DEBUG__ )
         hbqt_debug( "  DEL release_QSyntaxHighlighter" );
      #endif
   }
}
...
void * gcAllocate_QSyntaxHighlighter( void * pObj )
{
   ...
   #if defined( __HB_DEBUG__ )
      hbqt_debug( "          new_QSyntaxHighlighter          %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
   #endif
   return( p );
}
