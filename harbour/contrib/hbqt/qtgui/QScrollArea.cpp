...
QT_G_FUNC( release_QScrollArea )
{
   QGC_POINTER_QScrollArea * p = ( QGC_POINTER_QScrollArea * ) Cargo;

   HB_TRACE( HB_TR_DEBUG, ( "release_QScrollArea                  p=%p", p));
   HB_TRACE( HB_TR_DEBUG, ( "release_QScrollArea                 ph=%p pq=%p", p->ph, (void *)(p->pq)));
   ...
   if( p && p->ph && p->pq )
   {
      ...
      if( ( QString ) m->className() != ( QString ) "QObject" )
      {
         ...
         p->ph = NULL;
         HB_TRACE( HB_TR_DEBUG, ( "release_QScrollArea                 Object deleted!" ) );
         #if defined( __HB_DEBUG__ )
            hbqt_debug( "  YES release_QScrollArea                 %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
         #endif
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "release_QScrollArea                 Object Name Missing!" ) );
         #if defined( __HB_DEBUG__ )
            hbqt_debug( "  NO  release_QScrollArea" );
         #endif
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "release_QScrollArea                 Object Allready deleted!" ) );
      #if defined( __HB_DEBUG__ )
         hbqt_debug( "  DEL release_QScrollArea" );
      #endif
   }
}
...
void * gcAllocate_QScrollArea( void * pObj )
{
   ...
   #if defined( __HB_DEBUG__ )
      hbqt_debug( "          new_QScrollArea                 %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
   #endif
   return( p );
}
