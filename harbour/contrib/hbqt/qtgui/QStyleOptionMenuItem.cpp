...
hb_snprintf( str, sizeof(str), "release_QStyleOptionMenuItem        %i B %i KB", ( int ) hb_xquery( 1001 ), hb_getMemUsed() );  OutputDebugString( str );
#endif
   void * ph = ( void * ) Cargo;
   if( ph )
   {
      ( ( QStyleOptionMenuItem * ) ph )->~QStyleOptionMenuItem();
      ph = NULL;
   }
   else
   {
#if defined(__debug__)
hb_snprintf( str, sizeof(str), "! ph____QStyleOptionMenuItem" );  OutputDebugString( str );
#endif
   }
}
...
HB_FUNC( QT_QSTYLEOPTIONMENUITEM )
{
   ...
   void * pObj = NULL;
#if defined(__debug__)
hb_snprintf( str, sizeof(str), "   GC:  new QStyleOptionMenuItem        %i B %i KB", ( int ) hb_xquery( 1001 ), hb_getMemUsed() );  OutputDebugString( str );
#endif
...
{
   hb_retptrGC( hbqt_ptrTOgcpointer( new QFont( hbqt_par_QStyleOptionMenuItem( 1 )->font ), release_QFont ) );
}
...
{
   hb_retptrGC( hbqt_ptrTOgcpointer( new QIcon( hbqt_par_QStyleOptionMenuItem( 1 )->icon ), release_QIcon ) );
}
...
{
   hb_retptrGC( hbqt_ptrTOgcpointer( new QRect( hbqt_par_QStyleOptionMenuItem( 1 )->menuRect ), release_QRect ) );
}
