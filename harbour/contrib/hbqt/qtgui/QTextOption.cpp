...
QT_G_FUNC( hbqt_gcRelease_QTextOption )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTextOption                  p=%p", p ) );
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTextOption                 ph=%p", p->ph ) );

   if( p && p->ph )
   {
      delete ( ( QTextOption * ) p->ph );
      p->ph = NULL;
      HB_TRACE( HB_TR_DEBUG, ( "YES hbqt_gcRelease_QTextOption                 Object deleted! %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "DEL hbqt_gcRelease_QTextOption                 Object Already deleted!" ) );
   }
}
...
void * hbqt_gcAllocate_QTextOption( void * pObj )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "          new_QTextOption                 %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   return( p );
}
