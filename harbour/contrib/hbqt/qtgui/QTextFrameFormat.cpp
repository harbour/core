...
QT_G_FUNC( release_QTextFrameFormat )
{
   ...
   if( p && p->ph )
   {
      ...
      HB_TRACE( HB_TR_DEBUG, ( "release_QTextFrameFormat            Object deleted!" ) );
         ...
         hbqt_debug( "  YES release_QTextFrameFormat            %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
      #endif
   }
   else
   {
      ...
      #if defined( __HB_DEBUG__ )
         hbqt_debug( "  DEL release_QTextFrameFormat" );
      #endif
   }
}
...
void * gcAllocate_QTextFrameFormat( void * pObj )
{
   ...
   #if defined( __HB_DEBUG__ )
      hbqt_debug( "          new_QTextFrameFormat            %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
   #endif
   return( p );
}
