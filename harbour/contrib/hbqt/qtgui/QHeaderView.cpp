...
QT_G_FUNC( hbqt_gcRelease_QHeaderView )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QHeaderView                  p=%p", p));
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QHeaderView                 ph=%p pq=%p", p->ph, (void *)(p->pq)));
   ...
   if( p && p->ph && p->pq )
   {
      ...
      if( ( QString ) m->className() != ( QString ) "QObject" )
      {
         ...
         p->ph = NULL;
         HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QHeaderView                 Object deleted! %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "NO hbqt_gcRelease_QHeaderView                 Object Name Missing!" ) );
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "DEL hbqt_gcRelease_QHeaderView                 Object Already deleted!" ) );
   }
}
