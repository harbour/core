...
QT_G_FUNC( hbqt_gcRelease_QTextCharFormat )
{
   ...
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTextCharFormat              p=%p", p ) );
   HB_TRACE( HB_TR_DEBUG, ( "hbqt_gcRelease_QTextCharFormat             ph=%p", p->ph ) );

   if( p && p->ph )
   {
      delete ( ( QTextCharFormat * ) p->ph );
      p->ph = NULL;
      HB_TRACE( HB_TR_DEBUG, ( "YES hbqt_gcRelease_QTextCharFormat             Object deleted! %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "DEL hbqt_gcRelease_QTextCharFormat             Object Already deleted!" ) );
   }
}

void * hbqt_gcAllocate_QTextCharFormat( void * pObj )
{
   QGC_POINTER * p = ( QGC_POINTER * ) hb_gcAllocate( sizeof( QGC_POINTER ), hbqt_gcFuncs() );

   p->ph = pObj;
   p->func = hbqt_gcRelease_QTextCharFormat;
   HB_TRACE( HB_TR_DEBUG, ( "          new_QTextCharFormat             %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   return p;
}
...
HB_FUNC( QT_QTEXTCHARFORMAT )
{
   ...
   hb_retptrGC( hbqt_gcAllocate_QTextCharFormat( pObj ) );
}
/*
...
{
   hb_retptrGC( hbqt_gcAllocate_QStringList( new QStringList( hbqt_par_QTextCharFormat( 1 )->anchorNames() ) ) );
}
...
{
   hb_retptrGC( hbqt_gcAllocate_QFont( new QFont( hbqt_par_QTextCharFormat( 1 )->font() ) ) );
}
...
{
   hb_retptrGC( hbqt_gcAllocate_QPen( new QPen( hbqt_par_QTextCharFormat( 1 )->textOutline() ) ) );
}
...
{
   hb_retptrGC( hbqt_gcAllocate_QColor( new QColor( hbqt_par_QTextCharFormat( 1 )->underlineColor() ) ) );
}
