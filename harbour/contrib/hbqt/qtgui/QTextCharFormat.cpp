...
QT_G_FUNC( release_QTextCharFormat )
{
   ...
   if( p && p->ph )
   {
      ...
      HB_TRACE( HB_TR_DEBUG, ( "release_QTextCharFormat             Object deleted!" ) );
         ...
         hbqt_debug( "  YES release_QTextCharFormat             %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
      #endif
   }
   else
   {
      ...
      #if defined( __HB_DEBUG__ )
         hbqt_debug( "  DEL release_QTextCharFormat" );
      #endif
   }
}
...
void * gcAllocate_QTextCharFormat( void * pObj )
{
   ...
   #if defined( __HB_DEBUG__ )
      hbqt_debug( "          new_QTextCharFormat             %i B %i KB", ( int ) hb_xquery( 1001 ), hbqt_getmemused() );
   #endif
   return( p );
}
