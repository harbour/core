...
QT_G_FUNC( hbqt_gcRelease_QStyleOptionHeader )
{
   ...
   if( p && p->bNew )
   {
      if( p->ph )
      {
         ...
         HB_TRACE( HB_TR_DEBUG, ( "YES_rel_QStyleOptionHeader         ph=%p %i B %i KB", p->ph, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
         p->ph = NULL;
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "DEL_rel_QStyleOptionHeader          Object already deleted!" ) );
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "PTR_rel_QStyleOptionHeader          Object not created with - new" ) );
      p->ph = NULL;
   }
}
...
void * hbqt_gcAllocate_QStyleOptionHeader( void * pObj, bool bNew )
{
   ...
   if( bNew )
   {
      HB_TRACE( HB_TR_DEBUG, ( "   _new_QStyleOptionHeader         ph=%p %i B %i KB", pObj, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   return p;
}
