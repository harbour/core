...
QT_G_FUNC( hbqt_gcRelease_QStyleOptionViewItem )
{
   ...
   if( p && p->bNew )
   {
      if( p->ph )
      {
         ...
         HB_TRACE( HB_TR_DEBUG, ( "YES_rel_QStyleOptionViewItem       ph=%p %i B %i KB", p->ph, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
         p->ph = NULL;
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "DEL_rel_QStyleOptionViewItem        Object already deleted!" ) );
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "PTR_rel_QStyleOptionViewItem        Object not created with - new" ) );
      p->ph = NULL;
   }
}
...
void * hbqt_gcAllocate_QStyleOptionViewItem( void * pObj, bool bNew )
{
   ...
   if( bNew )
   {
      HB_TRACE( HB_TR_DEBUG, ( "   _new_QStyleOptionViewItem       ph=%p %i B %i KB", pObj, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   return p;
}
...
HB_FUNC( QT_QSTYLEOPTIONVIEWITEM )
{
   ...
   hb_retptrGC( hbqt_gcAllocate_QStyleOptionViewItem( pObj, true ) );
}
/*
