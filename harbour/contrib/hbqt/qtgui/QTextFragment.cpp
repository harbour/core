...
QT_G_FUNC( hbqt_gcRelease_QTextFragment )
{
   ...
   if( p && p->bNew )
   {
      if( p->ph )
      {
         ...
         HB_TRACE( HB_TR_DEBUG, ( "YES_rel_QTextFragment              ph=%p %i B %i KB", p->ph, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
         p->ph = NULL;
      }
      else
      {
         HB_TRACE( HB_TR_DEBUG, ( "DEL_rel_QTextFragment               Object already deleted!" ) );
      }
   }
   else
   {
      HB_TRACE( HB_TR_DEBUG, ( "PTR_rel_QTextFragment               Object not created with - new" ) );
      p->ph = NULL;
   }
}
...
void * hbqt_gcAllocate_QTextFragment( void * pObj, bool bNew )
{
   ...
   if( bNew )
   {
      HB_TRACE( HB_TR_DEBUG, ( "   _new_QTextFragment              ph=%p %i B %i KB", pObj, ( int ) hb_xquery( 1001 ), hbqt_getmemused() ) );
   }
   return p;
}
