...
typedef struct _HB_CURL
{
   ...
   struct curl_slist *    pHTTPPOSTHEADERS;
...
#define HB_CURL_FORMADD_TYPE_NOTSUP     0
#define HB_CURL_FORMADD_TYPE_COPYSTRING 1
#define HB_CURL_FORMADD_TYPE_STRING     2
#define HB_CURL_FORMADD_TYPE_PTR        3
#define HB_CURL_FORMADD_TYPE_LONG       4
#define HB_CURL_FORMADD_TYPE_SLIST      5

static HB_BOOL hb_curl_formadd( PHB_CURL hb_curl, PHB_ITEM pArray )
{
   ...
   if( pArray && HB_IS_ARRAY( pArray ) )
   {
      ...
      struct curl_forms * forms = ( struct curl_forms * ) hb_xgrab( sizeof( struct curl_forms ) * ( ulArrayLen + 1 ) );

      for( ulPos = 0; ulPos < ulArrayLen; ulPos += 2, ++i )
      {
         CURLformoption option = CURLFORM_NOTHING;
         int type = HB_CURL_FORMADD_TYPE_NOTSUP;

         PHB_ITEM pOptValue = hb_arrayGetItemPtr( pArray, ulPos + 2 );

         switch( hb_arrayGetNI( pArray, ulPos + 1 ) )
         {
            case HB_CURLFORM_COPYNAME:       option = CURLFORM_COPYNAME;       type = HB_CURL_FORMADD_TYPE_COPYSTRING; break;
            case HB_CURLFORM_PTRNAME:        option = CURLFORM_PTRNAME;        type = HB_CURL_FORMADD_TYPE_PTR;        break;
            case HB_CURLFORM_COPYCONTENTS:   option = CURLFORM_COPYCONTENTS;   type = HB_CURL_FORMADD_TYPE_COPYSTRING; break;
            case HB_CURLFORM_PTRCONTENTS:    option = CURLFORM_PTRCONTENTS;    type = HB_CURL_FORMADD_TYPE_PTR;        break;
            case HB_CURLFORM_CONTENTSLENGTH: option = CURLFORM_CONTENTSLENGTH; type = HB_CURL_FORMADD_TYPE_LONG;       break;
            case HB_CURLFORM_FILECONTENT:    option = CURLFORM_FILECONTENT;    type = HB_CURL_FORMADD_TYPE_STRING;     break;
            case HB_CURLFORM_FILE:           option = CURLFORM_FILE;           type = HB_CURL_FORMADD_TYPE_STRING;     break;
            case HB_CURLFORM_BUFFER:         option = CURLFORM_BUFFER;         type = HB_CURL_FORMADD_TYPE_STRING;     break;
            case HB_CURLFORM_BUFFERPTR:      option = CURLFORM_BUFFERPTR;      type = HB_CURL_FORMADD_TYPE_PTR;        break;
            case HB_CURLFORM_BUFFERLENGTH:   option = CURLFORM_BUFFERLENGTH;   type = HB_CURL_FORMADD_TYPE_LONG;       break;
            case HB_CURLFORM_CONTENTTYPE:    option = CURLFORM_CONTENTTYPE;    type = HB_CURL_FORMADD_TYPE_STRING;     break;
            case HB_CURLFORM_CONTENTHEADER:  option = CURLFORM_CONTENTHEADER;  type = HB_CURL_FORMADD_TYPE_SLIST;      break;
            case HB_CURLFORM_FILENAME:       option = CURLFORM_FILENAME;       type = HB_CURL_FORMADD_TYPE_STRING;     break;
         }

         forms[ i ].option = option;
         forms[ i ].value = NULL;

         switch( type )
         {
            case HB_CURL_FORMADD_TYPE_PTR:
            case HB_CURL_FORMADD_TYPE_COPYSTRING:
               forms[ i ].value = hb_itemGetCPtr( pOptValue );
               break;
            case HB_CURL_FORMADD_TYPE_STRING:
               forms[ i ].value = hb_curl_StrHash( hb_curl, hb_itemGetCPtr( pOptValue ) );
               break;
            case HB_CURL_FORMADD_TYPE_LONG:
            {
               size_t * plong = ( size_t * ) &forms[ i ].value;
               *plong = ( size_t ) hb_itemGetNInt( pOptValue );
               break;
            }
            case HB_CURL_FORMADD_TYPE_SLIST:
               if( pOptValue && HB_IS_ARRAY( pOptValue ) )
               {
                  HB_SIZE ulPos;
                  HB_SIZE ulArrayLen = hb_arrayLen( pOptValue );

                  hb_curl_slist_free( &hb_curl->pHTTPPOSTHEADERS );
                  hb_curl->pHTTPPOSTHEADERS = NULL;

                  for( ulPos = 0; ulPos < ulArrayLen; ++ulPos )
                     hb_curl->pHTTPPOSTHEADERS = curl_slist_append( hb_curl->pHTTPPOSTHEADERS, hb_arrayGetCPtr( pOptValue, ulPos + 1 ) );

                  forms[ i ].value = ( char * ) hb_curl->pHTTPPOSTHEADERS;
               }
               break;
         }
      }
      ...
      bSuccess = ( curl_formadd( &hb_curl->pHTTPPOST_First,
...
*/
         ...
         case HB_CURLOPT_HTTPPOST:
         {
            ...
            if( pArray )
            {
               HB_SIZE ulPos;
               HB_SIZE ulArrayLen = hb_arrayLen( pArray );
               HB_BOOL bAllOk = HB_TRUE;

               for( ulPos = 0; ulPos < ulArrayLen; ++ulPos )
               {
                  if( ! hb_curl_formadd( hb_curl, hb_arrayGetItemPtr( pArray, ulPos + 1 ) ) )
                  {
                     bAllOk = HB_FALSE;
                     break;
                  }
