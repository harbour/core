            ...
            if (vstyle.styles[ll->styles[charInLine]].visible) {
               if (isControl) {
                  if (ll->chars[charInLine] == '\t') {
                     ...
                     int iCharInChars = ll->chars[charInLine]; // Pritpal
                     if (ctrlCharWidth[ iCharInChars ] == 0) {
                        //const char *ctrlChar = ControlCharacterString(ll->chars[charInLine]);
                        const char *ctrlChar = ControlCharacterString( iCharInChars );
                        // +3 For a blank on front and rounded edge each side:
                        //ctrlCharWidth[ll->chars[charInLine]] =
                        ctrlCharWidth[ iCharInChars ] =
                            surface->WidthText(ctrlCharsFont, ctrlChar, istrlen(ctrlChar)) + 3;
                     }
                     //ll->positions[charInLine + 1] = ctrlCharWidth[ll->chars[charInLine]];
                     ll->positions[charInLine + 1] = ctrlCharWidth[ iCharInChars ];
                  } else {
...
void Editor::NotifyStyleToNeeded(int endStyleNeeded) {
   SCNotification scn = scNotification();
