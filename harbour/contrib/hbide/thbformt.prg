...
CLASS HbCodeFormat
   ...
   METHOD SetParams( xParams )
   METHOD AddUserCommands( xParams )
   METHOD AddUserClauses( xParams )
   METHOD AddUserFunctions( xParams )

   ENDCLASS

/*----------------------------------------------------------------------*/

METHOD AddUserCommands( xParams ) CLASS HbCodeFormat

   IF hb_isChar( xParams ) .OR. hb_isArray( xParams )
      IF Right( ::cCommands, 1 ) != ","
         ::cCommands += ","
      ENDIF
      IF hb_isChar( xParams )
         ::cCommands += xParams
      ENDIF
      IF hb_isArray( xParams )
         aeval( xParams, {|e| ::cCommands += e + "," } )
      ENDIF
   ENDIF
...
METHOD AddUserClauses( xParams ) CLASS HbCodeFormat
...
METHOD AddUserFunctions( xParams ) CLASS HbCodeFormat
...
METHOD SetParams( xParams ) CLASS HbCodeFormat
   LOCAL i, aParams, cParam

   IF !empty( xParams )
      IF hb_isChar( xParams )
         IF hb_fileExists( xParams )
            ::ReadIni( xParams )
            RETURN Self
         ELSE
            aParams := hb_ATokens( StrTran( hb_MemoRead( xParams ), Chr( 13 ) ), Chr( 10 ) )
         ENDIF
      ELSEIF hb_isArray( xParams )
         aParams := xParams
      ELSE
         aParams := {}
      ENDIF
   ELSE
      // Try to load from hbformat.ini residing in the executable folder
      //xParam := hb_dirBase() + hb_osPathSep() + "hbformat.ini"
      xParams := hb_dirBase() + "hbformat.ini"
      aParams := hb_ATokens( StrTran( hb_MemoRead( xParams ), Chr( 13 ) ), Chr( 10 ) )
   ENDIF
...
METHOD New( xParams ) CLASS HbCodeFormat
...
METHOD Reformat( aFile ) CLASS HbCodeFormat
...
METHOD FormatLine( cLine, lContinued ) CLASS HbCodeFormat
...
METHOD ConvertCmd( cLine, nBegin, nEnd, lFirstOnly ) CLASS HbCodeFormat
...
METHOD ConvertFnc( cLine, nBegin, nEnd ) CLASS HbCodeFormat
...
METHOD ConvertBool( cLine, nBegin, nEnd ) CLASS HbCodeFormat
...
METHOD SetOption( cLine, i, aIni ) CLASS HbCodeFormat
...
METHOD ReadIni( cIniName ) CLASS HbCodeFormat

   LOCAL i, nLen, aIni, c

   IF hb_FileExists( cIniName )
      aIni := hb_ATokens( StrTran( hb_MemoRead( cIniName ), Chr( 13 ) ), Chr( 10 ) )
      nLen := Len( aIni )
      FOR i := 1 TO nLen
...
METHOD File2Array( cFileName ) CLASS HbCodeFormat

   LOCAL aFile, cBuffer

   IF hb_FileExists( cFileName )
      cBuffer := hb_MemoRead( cFileName )
      aFile := hb_ATokens( StrTran( cBuffer, Chr( 13 ) ), Chr( 10 ) )
      IF ::nEol < 0
         ::cEol := iif( chr( 13 ) + chr( 10 ) $ cBuffer, chr( 13 ) + chr( 10 ),  iif( chr( 10 ) $ cBuffer, chr( 10 ), hb_eol() ) )
      ENDIF
   ENDIF
...
METHOD Array2File( cFileName, aFile ) CLASS HbCodeFormat
