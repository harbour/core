                           ...
                           HB_BOOL bTrunc )
{
   ...
   {
      if( lOffset )
         hb_fsSeekLarge( hFile, lOffset, FS_SET );
      else if( bOpen )
...
HB_FUNC( FILESTR )
{
   if( HB_ISCHAR( 1 ) )
   {
      ...
      if( hFile != FS_ERROR )
      {
         HB_FOFFSET lFileSize = hb_fsSeekLarge( hFile, 0, FS_END );
         HB_FOFFSET lPos = hb_fsSeekLarge( hFile, ( HB_FOFFSET ) hb_parnint( 3 ), FS_SET );
         HB_ISIZ lLength;
