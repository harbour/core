...
METHOD getData() CLASS DataRef
   ...
   DO CASE
   CASE ::className == "EDIT"
      ::sl_editBuffer := WDW_GetMessageText( ::hWnd, WM_GETTEXT, ::bufferLength + 1 )
   ENDCASE
...
METHOD setData( xValue, mp2 ) CLASS DataRef
   ...
   CASE ::className == "LISTBOX"    /* Single Selection */
      IF !empty( ::sl_editBuffer )
         RETURN WDW_LbSetCurSel( ::hWnd, ::sl_editBuffer - 1 ) >= 0
      ENDIF
      RETURN .f.

   CASE ::className == "SysTreeView32"
      IF ::sl_editBuffer <> NIL .and. ::sl_editBuffer:hItem <> NIL
         WDW_TreeView_SelectItem( ::hWnd, ::sl_editBuffer:hItem )
      ENDIF

   CASE ::className == "EDIT"
      IF hb_isChar( ::sl_editBuffer )
         WDW_SendMessageText( ::hWnd, WM_SETTEXT, 0, ::sl_editBuffer )
      ENDIF
