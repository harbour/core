...
FUNCTION WvtMyBrowse_X( oCrt )
   ...
   #if 1
   USE ( cFileDbf ) NEW EXCLUSIVE VIA ( cRDD ) ALIAS 'TEST'
   IF NetErr()
      Alert( 'Cannot open in exclusive mode!' )
   ELSE
      USE
   ENDIF
   #endif

   USE ( cFileDbf ) NEW SHARED VIA ( cRDD ) ALIAS 'TEST'
   if NetErr()
      return nil
   ENDIF

   #if 0
   do WHILE !dbRelease( 'TEST'  )
   ENDDO
   alert( 'Released' )
   RETURN nil
   #endif
...
Function DynDlgProc_2( hDlg, nMsg, wParam, lParam )
   ...
   case WM_TIMER
      WDW_SetDlgItemText( hDlg, ID_EDT_TIME, Time() )
      exit
   ...
   case WM_COMMAND
      ...
      case wParam == ID_CHK_SATIS
         lClicked := ( WDW_IsDlgButtonChecked( hDlg,ID_CHK_SATIS ) == 1 )
         WDW_MessageBox( hDlg, iif( lClicked, "Satisfied", "UnSatisfied" ), "CheckBoxStatus" )

      case wParam == ID_RDO_XH
         WDW_MessageBox( hDlg, "(x)Harbour", "Compiler" )

      case wParam == ID_RDO_CLIP
         WDW_MessageBox( hDlg, "Clipper", "Compiler" )

      case wParam == ID_RDO_XBASE
         WDW_MessageBox( hDlg, "Xbase++", "Compiler" )

      case wParam == ID_MNU_FILE
         WDW_MessageBox( hDlg, "Execute Menu Action!", "File" )

      case wParam == ID_MNU_CONTROL
         WDW_MessageBox( hDlg, "Controls are from Windows!", "Controls" )

      case WDW_LoWord( wParam ) == ID_LST_LIST
         if WDW_HiWord( wParam ) == LBN_SELCHANGE
            nIndex  := WDW_SendMessage( WDW_GetDlgItem( hDlg, ID_LST_LIST ), LB_GETCURSEL, 0, 0 )
            cPrompt := space( 20 )
            WDW_SendMessage( WDW_GetDlgItem( hDlg, ID_LST_LIST ), LB_GETTEXT, nIndex, @cPrompt )
            WDW_MessageBox( hDlg, cPrompt, "ListBox" )
         endif

      case WDW_LoWord( wParam ) == ID_CMB_COMBO
         if WDW_HiWord( wParam ) == CBN_SELCHANGE
            nIndex  := WDW_SendMessage( WDW_GetDlgItem( hDlg, ID_CMB_COMBO ), CB_GETCURSEL, 0, 0 )
            cPrompt := space( 20 )
            WDW_SendMessage( WDW_GetDlgItem( hDlg, ID_CMB_COMBO ), CB_GETLBTEXT, nIndex, @cPrompt )
            WDW_MessageBox( hDlg, cPrompt, "Combo Box" )
         endif
   ...
   case WM_CTLCOLOREDIT
      if ( WDW_GetDlgItem( hDlg,ID_MLE ) == lParam )
         WDW_SetTextColor( wParam, RGB( 0,0,255 ) )
         WDW_SetBkColor( wParam, RGB( 255,255,200 ) )
         return ( 1 )
      elseif ( WDW_GetDlgItem( hDlg,ID_EDT_TEXT ) == lParam )
         WDW_SetTextColor( wParam, RGB( 255,255,255 ) )
         WDW_SetBkColor( wParam, RGB( 10,200,45 ) )
         Return ( 1 )
      endif
   ...
   case WM_CTLCOLORSTATIC
      if ( WDW_GetDlgItem( hDlg,ID_STA_TEXT ) == lParam )
         WDW_SetTextColor( wParam, RGB( 255,255,255 ) )
         Return ( 1 )
      endif
      exit
   ...
   case WM_INITDIALOG
      WDW_SetTimer( hDlg, 5001, 1000 ) // 1 sec
      ...
      if len( t_ahFonts ) > 0
         WDW_SendMessage( WDW_GetDlgItem( hDlg, ID_MLE ), WM_SETFONT, t_ahFonts[ 1 ], 0 )
      endif

      if t_hIcon == nil
         t_hIcon := WDW_LoadIcon( "vr_1.ico" )
      endif
      if t_hIcon != nil .or. t_hIcon != 0
         WDW_SendMessage( WDW_GetDlgItem( hDlg, ID_ICO_VOUCH ), STM_SETIMAGE, IMAGE_ICON, t_hIcon )
      endif

      /*
      if t_hImage == nil
         t_hImage := WDW_LoadImage( "vouch1.bmp", 2 )
      endif
      if t_hImage != nil .and. t_hImage != 0
         WDW_SendMessage( WDW_GetDlgItem( hDlg, ID_STA_IMAGE ), STM_SETIMAGE, IMAGE_BITMAP, t_hImage )
      endif
      */
      WDW_SetDlgItemText( hDlg, ID_MLE      , GetEditText() )
      WDW_CheckDlgButton( hDlg, ID_CHK_SATIS, .t.           )

      WDW_CheckRadioButton( hDlg, ID_RDO_XH, ID_RDO_XBASE, ID_RDO_XH )
   ...
   case WM_DESTROY
      ...
      //
      cText := WDW_GetDlgItemText( hDlg, ID_MLE )
      cText := nil
      exit
...
EXIT PROCEDURE CleanHandles()
   ...
   for i := 1 to len( t_ahFonts )
      WDW_DeleteObject( t_ahFonts[ i ] )
   next

   if t_hIcon != nil
      WDW_DeleteObject( t_hIcon )
   endif

   if t_hImage != nil
      WDW_DeleteObject( t_hImage )
   endif
...
FUNCTION DrawSlide( hDlg, nSlide )
   ...
   hDC   := WDW_GetDC( hDlg )
   aRect := WDW_GetClientRect( hDlg )

   WDW_Rectangle( hDC, aRect[ 1 ]+10, aRect[ 2 ]+10, aRect[ 3 ]-10, aRect[ 4 ]-10 )
   WDW_DrawImage( hDC, aRect[ 1 ]+10, aRect[ 2 ]+10, aRect[ 3 ] - aRect[ 1 ] -20, ;
                                  aRect[ 4 ] - aRect[ 2 ] - 20, t_aSlides[ nSlide ] )

   WDW_ReleaseDC( hDlg,hDC )
...
/*
 *
 *                  Some More Solutions from my ERP Vouch
 *
 */
/*----------------------------------------------------------------------*/
...
function GetSome( msg, vrb, pass, pic, set_, wh, vl )
   Local screen, l, oWin, s, i, nMaxLen, nLenMsg, nLenVrb, vlen, pnt_, clr, r, nLen, oldCrt, oCrt
   Local t       := maxrow()-7
   Local b       := maxrow()-3
   Local nLen1   := 1
   Local GetList := {}
   ...
   oCrt := WvgCrt():new( , , { t,l }, { b-t, r-l }, , .t. )
   oCrt:lModal := .t.
   oCrt:create()
   hb_gtInfo( HB_GTI_ICONFILE, 'EXCLAIM' )
   hb_gtInfo( HB_GTI_WINTITLE, '  Info Required!' )
   SetColor( 'N/W' )
   CLS
   ...
   return vrb
//----------------------------------------------------------------------//
function GetRange( type, msg, dBgn, dEnd )
   local getlist := {}
   local scr, nMsgRow, nGetRow, b, e,nColMsgB,nColMsgE, oCrt, oOCrt
   local cBgn,cEnd,nGetCol1, nGetCol2, nLenGet, nLenWin
   local cPic, nMsgCol1, nMsgCol2,nColFrom
   LOCAL nT, nL, nB, nR

   DEFAULT type TO 'N'
   DEFAULT msg  TO if( type=='N', 'Required Range','Required Date Range' )

   b := if( type == 'N', 0, if( dBgn==nil, date(), dBgn ) )
   e := if( type == 'N', 0, if( dEnd==nil, date(), dEnd ) )

   cBgn     := 'Begining ' + if( type == 'N', 'Value', 'Date' )
   cEnd     := 'Ending '   + if( type == 'N', 'Value', 'Date' )
   nLenGet  := if( type == 'N', 12, 8 )
   cPic     := if( type == 'N', '@Z 99999999.999','' )

   nLenWin  := 2+len( cBgn ) + 1 + nLenGet + 2 + len( cEnd ) + 1 + nLenGet + 2
   nGetCol1 := 2+len( cBgn ) + 2
   nGetCol2 := nGetCol1 + nLenGet + 2 + len( cEnd )
   nMsgCol1 := 2
   nMsgCol2 := 2 + len( cBgn ) + 1 + nLenGet + 2
   nGetRow  := 1

   nColFrom := int( ( maxcol()-nLenWin )/2 )
   nMsgRow  := 13
   nGetRow  := 15

   nGetCol1 := nColFrom+nGetCol1   ; nGetCol2 := nColFrom+nGetCol2
   nMsgCol1 := nColFrom+nMsgCol1+1 ; nMsgCol2 := nColFrom+nMsgCol2

   nT := 12-1; nL := nColFrom; nB := 17-2; nR := nL + nLenWin+1

   /*  Create new modal dialog  */
   B_CRT nT, nL, nB, nR TITLE msg MODAL INTO oCrt

   /*  Realign coordinates  */
   nT := 0; nL := 0; nB := maxrow(); nR := maxcol()
   ...
   DispBox( nT, nL, nB, nR, '         ', 'N/B' )
   SetColor( 'w+/bg,N/GR*,,,n/w*' )

   Wvg_BoxRecessed( nGetRow, nGetCol1, nGetRow, nGetCol1+7 )
   Wvg_BoxRecessed( nGetRow, nGetCol2, nGetRow, nGetCol2+7 )

   DispOutAt( nGetRow, nMsgCol1, cBgn, 'gr+/b' )
   DispOutAt( nGetRow, nMsgCol2, cEnd, 'gr+/b' )

   SetCursor( 1 )

   @ nGetRow,nGetCol1 GET b VALID !empty(b) PICTURE cPic
   @ nGetRow,nGetCol2 GET e VALID e >= b    PICTURE cPic
