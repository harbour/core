...
METHOD setPos( aPos ) CLASS WvgSysWindow
   ...
   WDW_SetWindowPosition( ::hWnd, aPos[ 1 ], aPos[ 2 ], .f. )
...
METHOD wndProc( hWnd, nMessage, nwParam, nlParam ) CLASS WvgFontDialog
   ...
   CASE nMessage == WM_INITDIALOG
      ...
      IF !empty( ::title )
         WDW_setWindowText( ::hWnd, ::title )
      ENDIF
      IF !( ::buttonCancel )
         WDW_EnableWindow( WDW_GetDlgItem( ::hWnd,IDCANCEL ), .f. )
      ENDIF
      IF !( ::buttonApply )
         WDW_EnableWindow( WDW_GetDlgItem( ::hWnd,1026 ), .f. )
      ENDIF
      IF !( ::buttonHelp )
         WDW_EnableWindow( WDW_GetDlgItem( ::hWnd,1038 ), .f. )
      ENDIF
      IF !( ::strikeOut )
         WDW_EnableWindow( WDW_GetDlgItem( ::hWnd,1040 ), .f. )
      ENDIF
      IF !( ::underscore )
         WDW_EnableWindow( WDW_GetDlgItem( ::hWnd,1041 ), .f. )
      ENDIF
      IF !( ::name )
         WDW_EnableWindow( WDW_GetDlgItem( ::hWnd,1136 ), .f. )
      ENDIF
      IF !( ::style )
         WDW_EnableWindow( WDW_GetDlgItem( ::hWnd,1137 ), .f. )
      ENDIF
      IF !( ::size )
         WDW_EnableWindow( WDW_GetDlgItem( ::hWnd,1138 ), .f. )
      ENDIF

      IF ::aPos[ 1 ] > 0 .OR. ::aPos[ 2 ] > 0
         aRect := WDW_GetWindowRect( ::hWnd )
         WDW_MoveWindow( ::hWnd, ::aPos[ 1 ], ::aPos[ 2 ], aRect[3]-aRect[1], aRect[4]-aRect[2], .f. )
      ENDIF

      RETURN 1

   CASE nMessage == WM_COMMAND
      nL := WDW_LoWord( nwParam )
      nH := WDW_HiWord( nwParam )
