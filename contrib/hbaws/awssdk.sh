...
cmake -S $AWS_SDK_ROOT/src -B $AWS_SDK_ROOT/build -DCMAKE_INSTALL_PREFIX=$AWS_SDK_ROOT/$COMPILER/$BUILD -DCMAKE_BUILD_TYPE=$BUILD -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DBUILD_ONLY="s3" -DENABLE_TESTING=OFF -DENABLE_ZLIB_REQUEST_COMPRESSION=OFF -DAWS_SDK_WARNINGS_ARE_ERRORS=OFF -DBUILD_SHARED_LIBS=OFF
if [ $? -ne 0 ]; then
