...
PROCEDURE Main()
   ...
   HBTEST hb_ed25519_create_keypair( @public_key, @private_key )                    IS NIL
   ...
   HBTEST hb_ed25519_sign( @signature )                                             IS "E 1 BASE 3013 Argument error (HB_ED25519_SIGN) OS:0 #:0 F:S"
   message := hb_MemoRead( __FILE__ )
   HBTEST hb_ed25519_sign( @signature, message, public_key, private_key )           IS NIL
...
#if 0
   HBTEST hb_ed25519_key_exchange( @shared_secret, public_key, private_key )        IS NIL
