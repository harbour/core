...
static void i_update_scroll(GtkAdjustment *adjust, GtkWidget *scroller, const real32_t view_size, const real32_t area_size, const bool_t visible)
{
...
static void i_update_scrolls(OSView *view)
{
    cassert_no_null(view);
    if (view->hadjust != NULL)
        i_update_scroll(view->hadjust, view->hscroll, view->clip_width, view->area_width, view->hvisible);

    if (view->vadjust != NULL)
        i_update_scroll(view->vadjust, view->vscroll, view->clip_height, view->area_height, view->vvisible);
}
...
void osview_scroller_visible(OSView *view, const bool_t horizontal, const bool_t vertical)
{
    cassert_no_null(view);
    if (view->hscroll != NULL)
        view->hvisible = horizontal;
    if (view->vscroll != NULL)
        view->vvisible = vertical;
