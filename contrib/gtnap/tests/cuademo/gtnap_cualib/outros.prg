...
// #INCLUDE "xx.ch"
// #INCLUDE "inkey.ch"
// #INCLUDE "janela.ch"          // para usar os pseudo m�todos do objeto janela
// #INCLUDE "setcurs.ch"
// #INCLUDE "cua.ch"
// #INCLUDE "common.ch"
// #INCLUDE "mousecua.ch"
...
// * Tirar c�pia do vetor, pois ser� alterado aqui dentro...
...
// IF SOB_MODO_GRAFICO()   // reservar 4 colunas para mostrar a imagem � esquerda
...
//     #if defined(__PLATFORM__WINDOWS) || defined(__PLATFORM__Windows)
//        IF SOB_MODO_GRAFICO()
//           * Coordenadas para a WVW_PAINT (3 par�metro da AddGuiObjects)
...
//        N_Col := COL()
//     ENDIF
//     @ ROW(),COL() SAY VC_Menu[N_Cont]+SPACE(02) // espa�amento entre botoes
//     *
...
//                   REPL(CHR(K_RIGHT),N_PosBotao-1)+;  // deslocar at� a selecionada
...
// *********************
// *
// * -> Retira numa string a posi��o desejada de acordo com os delimitadores
// *
...
//    * Janela fecha sozinha ap�s algum tempo.
//    *
//    InkeyX(N_Segundos)
//    Destrua VX_Janela
// ELSE
//    SETPOS(ROW(),COL()-1)       // retroceder cursor em 1 posi��o
// ENDIF
// *
// SET(_SET_CURSOR,N_CursorAnt)          // restaurar modo do cursor
// *
...
// *
// N_Largura := 0                       // procurar maior t�tulo
// AEVAL(VC_Subtitulo,{|C_Elemento|;
//                    N_Largura := MAX(N_Largura,LEN(C_Elemento))} )
// *
// * + 1 = espa�o � esquerda do botao (acrescentado na TecFunc)
// * + 1 = espa�o � direita do botao (acrescentado na TecFunc)
// * + 2 = espa�o entre botoes
// *
// N_LargAux  := 0                      // largura das teclas de fun��es
// AEVAL(VC_TxtBotoes,{|C_Elemento|;
//                     N_LargAux := N_LargAux + LEN(C_Elemento) + 1 + 1 + 2 })
// N_Largura := MAX(N_LargAux,N_Largura)
// *
// * + 2 = espa�o entre op�oes
// *
...
// *
// * + 2 = colunas (do box)
// * + 2 = espa�amentos laterais (um de cada lado)
...
// *
// * + 2 = tra�os superior e inferior (do box)
// * + 1 = espa�amento inferior
// * N�O foi reservado espa�o SCROLLBAR HORIZONTAL / AREA DE MENSAGEM
// *
// N_Altura := 2+;       // linhas do box
//             IIF(EMPTY(VC_Menu),0,2)+;
//             IIF(EMPTY(VC_TxtBotoes),0,2)
// IF SOB_MODO_GRAFICO()  // reservar um m�nimo de 3 linhas para exibir imagem
...
// DEFAULT C_Cabec_x     TO "Informa��o"
...
// LOCAL N_OPCAO
// DEFAULT N_DEFAULT TO 1
// N_OPCAO := PERGUN(C_SubCabec,{"Sim","N�o"},N_DEFAULT,,"Confirma��o")
// RETURN (N_OPCAO==1)
// *
// ***********
...
//    *
//    * Subir o sublinhado, para "descolar" da margem inferior do bot�o.
//    *
//    IF N_TelaHeight >= 1024 .OR. ;  // resolu��o VERTICAL
//       N_TelaHeight >=  960
//       N_DeslocaVertical := -1
//       N_AlturaEmPixels  :=  2
...
//       * seja a mesma cor de fundo padr�o do windows.
...
//       * terminais remotos com somente 256 dispon�veis.
...
//       WVW_SetPalette(V_PALETTE)
    ...
    //   WvW_SetAltF4Close( .F. )
    //   *
    //   * ATEN��O: O arquivo GTWVW.PDF documenta que a WvW_SetMouseMove() tem
    //   *          DOIS par�metros. Ver c�digo fonte da GTWVW para ter certeza
    //   *          quantos par�metros possui, e ajustar o c�digo abaixo, se necess�rio.
    //   *
    //   * ATEN��O: � poss�vel que a linha abaixo n�o esteja fazendo nada,
    //   *          caso a fun��o tenha realmente DOIS par�metros.
    //   WvW_SetMouseMove( .T. )  // ATEN��O: Linha provavelmente com BUG !
    //   *
    //   * Colocar um intervalo de clique t�o pequeno, que o double-click
    //   * ser� interpretado como 2 cliques normais.
    //   WvW_SetMaxBMcache( 50 )  // os bitmaps s�o geralmente pequenos
    //   WvW_SetPointer( 0, 1 )
...
//       LOCAL N_HeightMin   // altura  m�nima que, com certeza, cabe na menor resolu��o
//       LOCAL N_WidthMin    // largura m�nima que, com certeza, cabe na menor resolu��o
//       LOCAL N_HeightMax   // altura  m�xima que ultrapassa a maior resolu��o
//       LOCAL N_WidthMax    // largura m�xima que ultrapassa a maior resolu��o
//       LOCAL N_MaxMaxRow
//       LOCAL N_MaxMaxCol
//       LOCAL C_Fonte
//       *
//       LOCAL L_Verbose := .F.  // S� colocar para .T. se for debugar esta rotina
//       *
//       * Resolu��es mais comuns:
...
//          * Testes pr�ticos mostraram que, em resolu��o vertical baixa (altura),
//          * o fonte "Courier New" apresenta um serrilhado que torna a leitura das letras
//          * dif�cil. Nesta situa��o, o "Lucida Console" tem apar�ncia bem melhor.
//          C_Fonte := "Lucida Console"
//       ELSE
//          * J� em resolu��o vertical alta, o "Courier New" n�o apresenta mais
//          * o serrilhado, e tem apar�ncia melhor.
//          * A desvantagem do "Lucida Console", neste caso, � que fica muito largo,
//          * com apar�ncia pesada e feia.
//          C_Fonte := "Courier New"
//       ENDIF
//       *
//       * - Os valores abaixo foram obtidos por via pr�tica, num computador com
//       *   Windows 7 e resolu��o 600 x 800 (SuperVGA)
//       * - Isto � importante, porque a SetMode() s� consegue funcionar se o
//       *   fonte escolhido efetivamente couber na resolu��o e tamanhos especificados
//       *
//       * Usar o valores m�nimos para permitir que a SetMode() mude a quantidade de
//       * linhas para 35 e colunas para 110, at� mesmo na resolu��o 600 X 800
//       N_HeightMin := 6
//       N_WidthMin  := 5
//       *
...
//       IF L_Verbose
//          ALERT("Com height m�nimo "+LTRIM(STR(N_HeightMin))+" e width m�nimo "+;
...
//       ENDIF
//       *
//       * Usar os valores m�ximos para ir reduzindo o tamanho at� que
//       * caiba na resolu��o da tela (a maior tela at� agora foi a 1024 x 1860)
//       N_HeightMax := 23+1    // em teste real, s� coube 23
//       N_WidthMax  := 15+1    // em teste real, s� coube 15
//       *
//       * Setar a maior altura de fonte poss�vel, que ainda caiba 35 linhas.
//       * Se WVW_MAXMAXROW() continuar com o mesmo valor, � porque n�o coube
//       * na tela, sendo necess�rio reduzir ainda mais a altura do fonte.
//       *
...
//       ENDIF
//       *
//       * Setar a maior altura de fonte poss�vel, que ainda caiba 110 colunas.
//       * Se WVW_MAXMAXCOL() continuar com o mesmo valor, � porque n�o coube
//       * na tela, sendo necess�rio reduzir ainda mais a largura do fonte.
//       *
...
//       *
//       /*
//       * USAR ESTE TRECHO DE C�DIGO S� EM AMBIENTE DE TESTE E NO CASO DE
//       * MUDAN�A DE FONTE, OU MUDAN�A DE TAMANHO M�NIMO DE RESULU��O.
//       #INCLUDE "intercep.ch"
//       *
...
//       LOCAL L_Verbose := .F.  // S� colocar para .T. se for debugar esta rotina
//       *
//       * No fonte "Lucida Console" a altera��o de 1 Height e 2 Width normalmente
//       * j� gera novo tamanho de fonte onde tanto a largura como a altura mudam,
//       * causando mudan�a no tamanho da tela (efeito de "encolhimento" ou "expans�o").
//       * Quando se mudou o fonte predominante para "Courier New", foi preciso
//       * aumentar a varia��o para 2 Height e 3 Width, para que a mudan�a
//       * de tamanho de fonte causasse tanto mudan�a na largura como na altura.
//       * Com isto, o salto na mudan�a de tamanho do fonte (e tela) passar� a ser maior,
//       * ao se usar a presente op��o.
//       *
//       IF L_Verbose
//          ALERT("O height atual � "+LTRIM(STR(N_HeightOld))+;
//                " e width atual � "+LTRIM(STR(N_WidthOld)))
//       ENDIF
//       *
//       IF N_Way == +1
...
//          * Letra ficou t�o pequena que torna quase imposs�vel a leitura
//          IF L_Verbose
//             ALERT("N�o se tentar� mudar para height "+LTRIM(STR(N_HeightNew))+;
//                   " e width "+LTRIM(STR(N_WidthNew)))
//          ENDIF
//       ELSE
//          IF L_Verbose
//             ALERT("Se tentar� mudar para height "+LTRIM(STR(N_HeightNew))+;
//                           " e width "+LTRIM(STR(N_WidthNew)))
//          ENDIF
//          *
//          N_MaxMaxRow := WVW_MAXMAXROW()
...
//             IF L_Verbose
//                ALERT("A mudan�a N�O foi realizada com sucesso")
//             ENDIF
//             Wvw_SetFont(,C_Fonte,N_HeightOld,N_WidthOld)
//             Wvw_PBSetFont(,C_Fonte,N_HeightOld,N_WidthOld)
//          ELSE
//             IF L_Verbose
//                ALERT("A mudan�a foi realizada com sucesso")
//             ENDIF
//             L_Mudou := .T.
//          ENDIF
//       ENDIF
...
//    ************************
//    * Propor��o entre a quantidade m�xima de linhas que caberia e a efetivamente usada
//    LOCAL N_Proporcao := MAXROW() / WVW_MAXMAXROW()
//    LOCAL N_TelaPrincipalHeight := INT(WVW_GetScreenHeight() * N_Proporcao) ;
//                                  - 21  // quantidade aproximada de pixels do t�tulo da janela
//    *
...
//    ***********************
//    * Propor��o entre a quantidade m�xima de colunas que caberia e a efetivamente usada
...
//       *     pela letra equivalente sem o acento (ex: "�" vem como "A").
//       *   - Isto ocorre em todos os acentos da PC850 que s�o diferentes na PC437
//       *     ("�����������ڧ").
//       *   - Tentou-se resolver esta quest�o usando WVW_GetClipboard(),
//       *     mas ocorreu o mesmo problema.
//       *
//       * COMO RESOLVER O PROBLEMA:
//       *   - Mudar a p�gina de c�digo da GTWVW "de volta" para a original (255-OEM para 1252-ANSI)
//       *   - Ler o texto do "clipboard" (vem na p�gina de c�digo do Windows)
//       *   - Voltar a p�gina de c�digo da GTWVW para a 255-OEM.
//       *   - NOTA: Por alguns milissegundos, a p�gina de c�digo da GTWVW fica errada, frente
//       *           aos PRGs e DBFs do aplicativo, mas a tela existente continua
//       *           exibindo os acentos corretamente. Ou seja, visualmente n�o fica errado,
//       *           nem por milissegundos...
//       *
//       * ATEN��O: Foi colocado um "zero" fixo no primeiro par�metro, pois NESTE EXE
//       *          parece que o "clipboard" do Windows se vincula � janela inicial da GTWVW,
//       *          mesmo que se esteja posicionado em uma tela subsequente.
//       * GRAVE: Mas, em EXE gerado sem a CUA, teve-se se informar a janela "do topo", ao inv�s
//       *        janela inicial. N�o tivemos tempo de investigar o motivo desta diferen�a
//       *        de comportamento...
//       #DEFINE N_WindowNum_Janela_Principal  0
//       WVW_SetCodePage(N_WindowNum_Janela_Principal,1252)  // 1252-ANSI
//       IF Version()=="Harbour 3.2.0dev (r1703241902)"
//          C_RETORNO := HB_AnsiToOEM(HB_GTINFO(HB_GTI_CLIPBOARDDATA))  // modo gr�fico
//       ELSEIF Version()=="Harbour 3.2.0dev (r2011030937)" .OR. Version()=="Harbour 3.2.0dev (r1704061005)" // PENDENTE_LINUX
//          C_RETORNO := HB_GTINFO(HB_GTI_CLIPBOARDDATA)  // modo gr�fico
//       ENDIF
...
//       *   - Se for usado, parte dos caracteres acentuados v�o errados
//       *     (v�o como caracteres de borda de boxes !). Isto ocorre em todos os
//       *     acentos da PC850 que s�o diferentes na PC437 ("�����������ڧ").
//       *   - A sa�da tempor�ria foi excluir do textos os referidos caracteres acentuados.
//       *       C_Texto := TROCA(C_Texto,"�����������ڧ",;
//       *                                "AAAAaEIOOOoU ")
//       *   - Tentou-se resolver esta quest�o usando WVW_SetClipboard() e a
//       *     WVW_PasteFromClipboard(), mas ocorreu o mesmo problema.
//       *
//       * COMO RESOLVER O PROBLEMA:
//       *   - Mudar a p�gina de c�digo da GTWVW "de volta" para a original (255-OEM para 1252-ANSI)
//       *   - Colar no "clipboard" um texto j� na p�gina de c�digo do Windows
//       *   - Voltar a p�gina de c�digo da GTWVW para a 255-OEM.
//       *   - NOTA: Por alguns milissegundos, a p�gina de c�digo da GTWVW fica errada, frente
//       *           aos PRGs e DBFs do aplicativo, mas a tela existente continua
//       *           exibindo os acentos corretamente. Ou seja, visualmente n�o fica errado,
...
//       WVW_SetCodePage(N_WindowNum_Janela_Principal,1252)  // 1252-ANSI
//       IF Version()=="Harbour 3.2.0dev (r1703241902)"
//          HB_GTINFO(HB_GTI_CLIPBOARDDATA,HB_OEMToAnsi(C_Texto))  // modo gr�fico
//       ELSEIF Version()=="Harbour 3.2.0dev (r2011030937)" .OR. Version()=="Harbour 3.2.0dev (r1704061005)" // ADAPTACAO_LINUX
//          HB_GTINFO(HB_GTI_CLIPBOARDDATA,C_Texto)  // modo gr�fico
//       ENDIF
//       WVW_SetCodePage(N_WindowNum_Janela_Principal,255)   //  255-OEM
//       #UNDEF N_WindowNum_Janela_Principal
//    #elif defined(__PLATFORM__LINUX)
