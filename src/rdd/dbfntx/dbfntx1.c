...
static void hb_ntxTagSetScope( LPTAGINFO pTag, HB_USHORT nScope, PHB_ITEM pItem )
{
   ...
   pScopeVal = HB_IS_BLOCK( pItem ) ? hb_vmEvalBlock( pItem ) : pItem;
...
static void hb_ntxTagRefreshScope( LPTAGINFO pTag )
{
   ...
   if( HB_IS_BLOCK( pTag->top.scopeItem ) )
   {
   ...
   if( HB_IS_BLOCK( pTag->bottom.scopeItem ) )
   {
...
static HB_BOOL hb_ntxOrdKeyAdd( LPTAGINFO pTag, PHB_ITEM pItem )
{
   ...
   if( pTag->Template && pItem && ! HB_IS_NIL( pItem ) )
   {
...
static HB_ERRCODE hb_ntxOrderCreate( NTXAREAP pArea, LPDBORDERCREATEINFO pOrderInfo )
{
   ...
   if( pForExp )
   {
      ...
      fOK = HB_IS_LOGICAL( pArea->dbfarea.area.valResult );
   ...
   if( pTag )
   {
      switch( uiIndex )
      {
         ...
         case DBOI_ISDESC:
            ...
            if( HB_IS_LOGICAL( pInfo->itmNewVal ) )
         ...
         case DBOI_FINDRECCONT:
            ...
            break;
         case DBOI_SCOPEEVAL:
            if( HB_IS_ARRAY( pInfo->itmNewVal ) &&
